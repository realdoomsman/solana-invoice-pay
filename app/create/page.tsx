'use client'

import { useState, useEffect } from 'react'
import { useRouter } from 'next/navigation'
import { nanoid } from 'nanoid'
import { generatePaymentWallet } from '@/lib/payment-wallet'
import { getCurrentUser } from '@/lib/auth'
import { SplitRecipient, Milestone } from '@/lib/types'

export default function CreatePayment() {
  const router = useRouter()
  const [user, setUser] = useState<{ walletAddress: string } | null>(null)
  
  // Basic fields
  const [amount, setAmount] = useState('')
  const [token, setToken] = useState('SOL')
  const [description, setDescription] = useState('')
  const [merchantWallet, setMerchantWallet] = useState('')
  
  // Payment type
  const [paymentType, setPaymentType] = useState<'simple' | 'split' | 'escrow' | 'goal'>('simple')
  
  // Split payment
  const [splitRecipients, setSplitRecipients] = useState<SplitRecipient[]>([
    { address: '', percentage: 100, name: '' }
  ])
  
  // NFT Receipt
  const [mintNFT, setMintNFT] = useState(false)
  
  // Escrow
  const [milestones, setMilestones] = useState<Milestone[]>([
    { id: nanoid(6), description: '', percentage: 100, status: 'pending' }
  ])
  
  // Goal
  const [goalAmount, setGoalAmount] = useState('')
  
  const [loading, setLoading] = useState(false)

  useEffect(() => {
    const currentUser = getCurrentUser()
    if (currentUser) {
      setMerchantWallet(currentUser.walletAddress)
      setUser(currentUser)
      setSplitRecipients([{ address: currentUser.walletAddress, percentage: 100, name: 'You' }])
    }
  }, [])

  const addSplitRecipient = () => {
    setSplitRecipients([...splitRecipients, { address: '', percentage: 0, name: '' }])
  }

  const removeSplitRecipient = (index: number) => {
    setSplitRecipients(splitRecipients.filter((_, i) => i !== index))
  }

  const updateSplitRecipient = (index: number, field: keyof SplitRecipient, value: string | number) => {
    const updated = [...splitRecipients]
    updated[index] = { ...updated[index], [field]: value }
    setSplitRecipients(updated)
  }

  const addMilestone = () => {
    setMilestones([...milestones, { id: nanoid(6), description: '', percentage: 0, status: 'pending' }])
  }

  const removeMilestone = (index: number) => {
    setMilestones(milestones.filter((_, i) => i !== index))
  }

  const updateMilestone = (index: number, field: string, value: string | number) => {
    const updated = [...milestones]
    updated[index] = { ...updated[index], [field]: value }
    setMilestones(updated)
  }

  const getTotalPercentage = () => {
    return splitRecipients.reduce((sum, r) => sum + Number(r.percentage), 0)
  }

  const createPaymentLink = () => {
    if (!amount || parseFloat(amount) <= 0) {
      alert('Please enter a valid amount')
      return
    }

    if (paymentType === 'split') {
      const total = getTotalPercentage()
      if (total !== 100) {
        alert(`Split percentages must equal 100% (currently ${total}%)`)
        return
      }
      if (splitRecipients.some(r => !r.address)) {
        alert('Please fill in all recipient addresses')
        return
      }
    }

    if (paymentType === 'escrow') {
      const total = milestones.reduce((sum, m) => sum + Number(m.percentage), 0)
      if (total !== 100) {
        alert(`Milestone percentages must equal 100% (currently ${total}%)`)
        return
      }
    }

    if (paymentType === 'goal' && (!goalAmount || parseFloat(goalAmount) <= 0)) {
      alert('Please enter a valid goal amount')
      return
    }

    setLoading(true)

    const paymentId = nanoid(10)
    const wallet = generatePaymentWallet()

    const paymentData = {
      id: paymentId,
      amount: parseFloat(amount),
      token,
      description,
      status: 'pending',
      createdAt: new Date().toISOString(),
      paymentWallet: wallet.publicKey,
      privateKey: wallet.privateKey,
      merchantWallet: merchantWallet,
      type: paymentType,
      splitRecipients: paymentType === 'split' ? splitRecipients : undefined,
      mintNFT,
      escrowEnabled: paymentType === 'escrow',
      milestones: paymentType === 'escrow' ? milestones : undefined,
      isGoal: paymentType === 'goal',
      goalAmount: paymentType === 'goal' ? parseFloat(goalAmount) : undefined,
      currentAmount: paymentType === 'goal' ? 0 : undefined,
      contributors: paymentType === 'goal' ? 0 : undefined,
    }

    const payments = JSON.parse(localStorage.getItem('payments') || '[]')
    payments.push(paymentData)
    localStorage.setItem('payments', JSON.stringify(payments))

    router.push(`/pay/${paymentId}`)
  }

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <button
            onClick={() => router.push('/')}
            className="text-blue-600 dark:text-blue-400 hover:underline mb-4"
          >
            ← Back
          </button>
          <h1 className="text-4xl font-bold text-slate-900 dark:text-white mb-2">
            Create Payment Link
          </h1>
          <p className="text-slate-600 dark:text-slate-400">
            Choose your payment type and configure options
          </p>
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-8">
          {/* Payment Type Selector */}
          <div className="mb-8">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">
              Payment Type
            </label>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <button
                onClick={() => setPaymentType('simple')}
                className={`p-4 rounded-lg border-2 transition-all ${
                  paymentType === 'simple'
                    ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20'
                    : 'border-slate-300 dark:border-slate-600 hover:border-blue-400'
                }`}
              >
                <div className="text-2xl mb-2">💳</div>
                <div className="font-semibold text-sm">Simple</div>
              </button>
              <button
                onClick={() => setPaymentType('split')}
                className={`p-4 rounded-lg border-2 transition-all ${
                  paymentType === 'split'
                    ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20'
                    : 'border-slate-300 dark:border-slate-600 hover:border-blue-400'
                }`}
              >
                <div className="text-2xl mb-2">🔀</div>
                <div className="font-semibold text-sm">Split</div>
              </button>
              <button
                onClick={() => setPaymentType('escrow')}
                className={`p-4 rounded-lg border-2 transition-all ${
                  paymentType === 'escrow'
                    ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20'
                    : 'border-slate-300 dark:border-slate-600 hover:border-blue-400'
                }`}
              >
                <div className="text-2xl mb-2">🔒</div>
                <div className="font-semibold text-sm">Escrow</div>
              </button>
              <button
                onClick={() => setPaymentType('goal')}
                className={`p-4 rounded-lg border-2 transition-all ${
                  paymentType === 'goal'
                    ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20'
                    : 'border-slate-300 dark:border-slate-600 hover:border-blue-400'
                }`}
              >
                <div className="text-2xl mb-2">🎯</div>
                <div className="font-semibold text-sm">Goal</div>
              </button>
            </div>
          </div>

          {/* Basic Fields */}
          <div className="space-y-5 mb-8">
            <div className="grid grid-cols-3 gap-4">
              <div className="col-span-2">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                  Amount {paymentType === 'goal' && '(per contribution)'}
                </label>
                <input
                  type="number"
                  step="0.01"
                  value={amount}
                  onChange={(e) => setAmount(e.target.value)}
                  placeholder="0.00"
                  className="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                  Token
                </label>
                <select
                  value={token}
                  onChange={(e) => setToken(e.target.value)}
                  className="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                >
                  <option value="SOL">SOL</option>
                  <option value="USDC">USDC</option>
                  <option value="USDT">USDT</option>
                </select>
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                Description
              </label>
              <textarea
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="What is this payment for?"
                rows={3}
                className="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white resize-none"
              />
            </div>
          </div>

          {/* Split Payment Fields */}
          {paymentType === 'split' && (
            <div className="mb-8 p-6 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <h3 className="font-semibold text-slate-900 dark:text-white mb-4">
                Split Recipients
              </h3>
              <div className="space-y-3">
                {splitRecipients.map((recipient, index) => (
                  <div key={index} className="flex gap-3">
                    <input
                      type="text"
                      placeholder="Wallet address"
                      value={recipient.address}
                      onChange={(e) => updateSplitRecipient(index, 'address', e.target.value)}
                      className="flex-1 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm font-mono"
                    />
                    <input
                      type="text"
                      placeholder="Name"
                      value={recipient.name}
                      onChange={(e) => updateSplitRecipient(index, 'name', e.target.value)}
                      className="w-32 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm"
                    />
                    <input
                      type="number"
                      placeholder="%"
                      value={recipient.percentage}
                      onChange={(e) => updateSplitRecipient(index, 'percentage', parseFloat(e.target.value) || 0)}
                      className="w-20 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm"
                    />
                    {splitRecipients.length > 1 && (
                      <button
                        onClick={() => removeSplitRecipient(index)}
                        className="px-3 py-2 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/50"
                      >
                        ✕
                      </button>
                    )}
                  </div>
                ))}
              </div>
              <div className="flex justify-between items-center mt-4">
                <button
                  onClick={addSplitRecipient}
                  className="text-sm text-blue-600 dark:text-blue-400 hover:underline"
                >
                  + Add Recipient
                </button>
                <span className={`text-sm font-semibold ${getTotalPercentage() === 100 ? 'text-green-600' : 'text-red-600'}`}>
                  Total: {getTotalPercentage()}%
                </span>
              </div>
            </div>
          )}

          {/* Escrow Milestones */}
          {paymentType === 'escrow' && (
            <div className="mb-8 p-6 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
              <h3 className="font-semibold text-slate-900 dark:text-white mb-4">
                Payment Milestones
              </h3>
              <div className="space-y-3">
                {milestones.map((milestone, index) => (
                  <div key={milestone.id} className="flex gap-3">
                    <input
                      type="text"
                      placeholder="Milestone description"
                      value={milestone.description}
                      onChange={(e) => updateMilestone(index, 'description', e.target.value)}
                      className="flex-1 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm"
                    />
                    <input
                      type="number"
                      placeholder="%"
                      value={milestone.percentage}
                      onChange={(e) => updateMilestone(index, 'percentage', parseFloat(e.target.value) || 0)}
                      className="w-20 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm"
                    />
                    {milestones.length > 1 && (
                      <button
                        onClick={() => removeMilestone(index)}
                        className="px-3 py-2 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/50"
                      >
                        ✕
                      </button>
                    )}
                  </div>
                ))}
              </div>
              <button
                onClick={addMilestone}
                className="text-sm text-purple-600 dark:text-purple-400 hover:underline mt-4"
              >
                + Add Milestone
              </button>
            </div>
          )}

          {/* Goal Amount */}
          {paymentType === 'goal' && (
            <div className="mb-8 p-6 bg-green-50 dark:bg-green-900/20 rounded-lg">
              <h3 className="font-semibold text-slate-900 dark:text-white mb-4">
                Funding Goal
              </h3>
              <input
                type="number"
                step="0.01"
                value={goalAmount}
                onChange={(e) => setGoalAmount(e.target.value)}
                placeholder="Target amount"
                className="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-green-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
              />
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-2">
                Accept multiple contributions until this goal is reached
              </p>
            </div>
          )}

          {/* NFT Receipt Option */}
          <div className="mb-8">
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                type="checkbox"
                checked={mintNFT}
                onChange={(e) => setMintNFT(e.target.checked)}
                className="w-5 h-5 text-blue-600 rounded"
              />
              <div>
                <span className="font-medium text-slate-900 dark:text-white">
                  Mint NFT Receipt
                </span>
                <p className="text-xs text-slate-600 dark:text-slate-400">
                  Create a collectible NFT as proof of payment
                </p>
              </div>
            </label>
          </div>

          {/* Create Button */}
          <button
            onClick={createPaymentLink}
            disabled={loading}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-4 px-6 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? 'Creating...' : 'Create Payment Link'}
          </button>
        </div>
      </div>
    </div>
  )
}
